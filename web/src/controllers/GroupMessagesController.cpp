 ...
 */
#include "controllers/GroupMessagesController.hpp"
...
/* Advanced-search group messages; mirrors Users/Groups/FilesController (shared
 * core + shared search_page.html). Rows link into the group and the sender's
 * profile via their "party" cells, so there is no row-level detail page. */
drogon::Task<drogon::HttpResponsePtr>
GroupMessagesController::list(drogon::HttpRequestPtr req)
{
	...
	data["entity"] = "group_messages";
	data["detail_base"] = "";     /* rows link via the group/sender "party" cells */
	data["search_error"] = "";
	...
	nlohmann::json result = co_await dao::search::run(
		db, dao::search::groupMessagesSchema(), std::move(sreq));
	...
	if (result.contains("error")) {
		...
		result = co_await dao::search::run(
			db, dao::search::groupMessagesSchema(), std::move(empty));
	}
